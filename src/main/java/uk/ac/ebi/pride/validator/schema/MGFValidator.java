 ...
 * @version $Id$
 */
public class MGFValidator extends GenericXmlSchemaValidator {
    ...
    public boolean support(File input) {
        try {
            ...
            return fileFormat != null && fileFormat.equals(MassSpecFileFormat.MGF);
        } catch (IOException e) {
        ...
        try {
            while ((line = in.readLine()) != null) {
                ...
                if (line.equals("BEGIN IONS")) {
                    if (inSpectrum)
                        response.add("Unexpected BEGIN IONS at line:" + lineNbr);
                    inSpectrum = true;
                } else if (line.equals("END IONS")) {
                    if (!inSpectrum)
                        response.add("Unexpected END IONS at line:" + lineNbr);
                    inSpectrum = false;
                } else if (!Pattern.matches("^\\d+\\.?\\d*[ \\t]\\d+\\.?\\d*E?-?\\d?", line)) {
                    if (line.length() > 0) {
                        boolean isOK = false;
                        ...
                        if (!isOK) {
                            response.add("Unexpected line in MGF:" + lineNbr + ". " + line);
                        }
                    }
                }
            }
